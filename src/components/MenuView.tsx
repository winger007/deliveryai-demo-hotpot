    ...
    <section className="min-w-0">
      ...
      <div className="mt-3 grid gap-4 sm:grid-cols-2">
        {visible.map((product, index) => {
          ...
          return (
            <article key={product.id} className="group animate-rise overflow-hidden rounded-3xl border border-charcoal-900/5 bg-white shadow-card transition hover:-translate-y-1 dark:border-rice-200/10 dark:bg-charcoal-700 dark:shadow-none" style={{ animationDelay: `${index * 40}ms` }}>
              <div className="relative h-40 overflow-hidden">
                ...
                {unavailable && <span className="absolute inset-0 flex items-center justify-center bg-white/55 text-lg font-extrabold text-charcoal-900 backdrop-blur-sm dark:bg-charcoal-700/70 dark:text-rice-50">{t('menu.sold_out')}</span>}
                {product.orderedCount && <span className="absolute bottom-3 left-3 flex items-center gap-1 rounded-full bg-white/90 px-2 py-1 text-xs font-bold text-chili-600 dark:bg-charcoal-700/90 dark:text-chili-500"><Check size={13} />{t('menu.ordered_table', { count: product.orderedCount })}</span>}
              </div>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => { if (!open) { setSelected(null); setShowRiskWarning(false) } }}>
        <DialogContent title={selected ? t(selected.name) : ''}>
          {selected && <>
            ...
            <div className="mt-5"><p className="mb-3 flex items-center gap-2 text-sm font-bold text-charcoal-900 dark:text-rice-50"><Users size={16} />{t('menu.who_ordered')}</p><div className="flex gap-2">{diners.map((name) => <button key={name} onClick={() => setDiner(name)} className={`rounded-full px-4 py-2 text-sm font-bold ${diner === name ? 'bg-amber-400 text-charcoal-900' : 'bg-white text-charcoal-500 dark:bg-charcoal-500 dark:text-rice-200/70'}`}>{name}</button>)}</div></div>
      ...
      <Dialog open={showRiskWarning} onOpenChange={(open) => !open && cancelSuperSpicy()}>
        <DialogContent title={t('menu.super_spicy_warning_title')}>
          <div className='mt-4 flex flex-col items-center gap-4'>
            ...
            <p className='text-center text-sm leading-6 text-charcoal-700 dark:text-rice-100'>{t('menu.super_spicy_warning')}</p>
