    ...
    <main className="mx-auto max-w-5xl px-4 py-6 lg:px-6 lg:py-8">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-5">
        ...
        <section className="rounded-3xl bg-white p-5 shadow-card lg:col-span-3 dark:bg-charcoal-700 dark:shadow-none">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-extrabold text-charcoal-900 dark:text-rice-50">{t('order.items_title')}</h2><p className="mt-1 text-xs text-charcoal-500 dark:text-rice-200/60">{t('order.order_no')}</p></div><span className="rounded-full bg-chili-50 px-3 py-1 text-xs font-bold text-chili-600 dark:bg-chili-500/10 dark:text-chili-500">{t('order.item_count', { count: items.length })}</span></div>
          <div className="mt-5 space-y-4">
            {items.map((item) => <div key={item.uid} className="flex gap-3 border-b border-charcoal-900/5 pb-4 last:border-0 dark:border-rice-200/10"><img src={item.image} alt={item.name} className="h-16 w-16 rounded-xl object-cover" /><div className="min-w-0 flex-1"><div className="flex justify-between gap-3"><p className="truncate font-bold text-charcoal-900 dark:text-rice-50">{item.name} <span className="font-normal text-charcoal-500 dark:text-rice-200/60">× {item.quantity}</span></p><strong className="text-charcoal-900 dark:text-rice-50">{money(item.price * item.quantity)}</strong></div><p className="mt-1 text-xs text-charcoal-500 dark:text-rice-200/60">{item.spec} · {t('order.ordered_by', { name: item.orderedBy })}</p><div className="mt-2 flex items-center justify-between"><span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-charcoal-700 dark:bg-amber-400/15 dark:text-amber-400">{t(`order.stage.${item.stage}.label`)}</span>{item.cancelState ? <span className="text-xs font-bold text-amber-500">{t('order.cancel_pending')}</span> : item.stage !== 'served' && <button onClick={() => onCancel(item.uid)} className="flex items-center gap-1 text-xs font-semibold text-charcoal-500 hover:text-chili-500 dark:text-rice-200/60 dark:hover:text-chili-500"><RotateCcw size={13} />{t('order.cancel')}</button>}</div></div></div>)}
          </div>
          <button className="mt-3 flex w-full items-center justify-between rounded-2xl bg-rice-100 p-4 text-left dark:bg-charcoal-500/50"><span><small className="block text-charcoal-500 dark:text-rice-200/60">{t('order.total')}</small><strong className="text-xl text-chili-500">{money(total)}</strong></span><span className="flex items-center gap-1 text-sm font-bold text-charcoal-900 dark:text-rice-50">{t('order.view_detail')}<ChevronRight size={16} /></span></button>
